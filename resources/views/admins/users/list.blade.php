            ...
            <div class="card card-custom">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    ...
                    <div class="card-toolbar">
                        <!--begin::Button-->
                        <a href="{{route('admins.users.addUser')}}" class="btn btn-primary font-weight-bolder">
                        ...
                        @if (!empty($data))
                            @foreach ($data as $key => $value)
                                <tr>
                                    ...
                                    <td>
                                        @can('changeStatus')
                                        <form action="{{route('admins.users.changeStatus')}}" method="post">
                                            @csrf
                                            <input type="hidden" value="{{$value->id}}" name="UserId">
                                            @if($value->status == 'activity')
                                                <button type="submit" class="btn btn-success" name="status" value="{{$value->status}}">{{$value->status}}</button>
                                            @else
                                                <button type="submit" class="btn btn-danger" name="status" value="{{$value->status}}">{{$value->status}}</button>
                                            @endif
                                ...
                                <span style="overflow: visible; position: relative; width: 125px;" class="d-flex">
                                    @can('updateUser')
                                    <a href="{{route('admins.users.updateUser', ['id' => $value->id])}}"
